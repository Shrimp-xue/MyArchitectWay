...
/**
 * @description: Blowfish加密算法Java实现
 * @author: shrimp
 * @date: Created in 2020-01-11 18:36
 * @version: 1.0
 */
public class Blowfish {
    ...
    public byte[] encrypt(final String input) {
        try {
            final byte[] bytes = input.getBytes(Constant.UTF_8);
    ...
    public String decryptString(final byte[] source) {
        try {
            return new String(decryptBytes(source), Constant.UTF_8);
        } catch (UnsupportedEncodingException e) {
            return null;
        }
    }
